...
const MessagingHub = () => {
    ...
    const fetchPotentialReceivers = async () => {
        try {
            const res = await api.get('/auth/users'); // Admin/Generic user fetch
            setReceivers(res.data.data.filter(u => u._id !== user._id));
        } catch (error) {
                            ...
                            {activeChat.messages.map((msg, idx) => (
                                <div
                                    ...
                                    style={{
                                        ...
                                        background: msg.role === 'user' ? 'var(--diamond-600)' : 'var(--surface-alt)',
